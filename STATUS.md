...
Etapa actual: STORY 6.6 COMPLETO. (A) Notifications reales: helper src/lib/notifications.ts, generación automática en session/resolve (documento_generado + sesion_completada), stripe/webhook (pago_procesado), usage.ts (saldo_bajo cuando balance cae de $5→<$5). Bell en header → /settings/notificaciones. Página notificaciones: historial real desde DB + mark-as-read al cargar. (B) Settings Account: perfil ya funcionaba, password reset via email. Auth/confirm ahora maneja type=recovery → redirige a /auth/reset-password (nueva página). Fix: botón "Tomar enviar" → "Enviar enlace". Delete account: /api/auth/delete-account elimina notifications→usages→invoices→balances→subscriptions→projects(cascade)→profiles→auth.users. (C) Advisor custom on-demand: botón en ConsejoPrincipalPropuesta → textarea → POST /api/advisors/generate-custom (Sonnet genera perfil, Haiku genera system_prompt, agrega al council). Badge "Personalizado" en cards. (D) Stripe webhook: logging detallado de todos los eventos. (E) Password reset E2E: 3/3 PASS. Pendiente SQL: nexo_custom_prompt. Pendiente: STRIPE_PRICE_* en Railway.
